...
new const kz_lj[] = "addons/amxmodx/configs/kz_longjumps2.txt";
...
public checkwrs()
{
	if(kz_lj2)
	{
		...
		formatex(ljname, charsmax(ljname), "%d_%s", dl_sources[src_num][dl_distance], dl_sources[src_num][dl_type]);
		if(!already_parsed(dl_sources[src_num][dl_name], ljname))
		{
			Check_Download_Demos( 1, true, true); 
		}
	}
	else
	{
	...
	if(file_exists(kz_lj))
	{
		...
		while (!feof(lj2_file))
		{
			...
			if(strlen(Line) > 0) 
			{
				...
				{
					src_num++;
					dl_sources[src_num][dl_distance] = str_to_num(ExplodedString[0]);
					formatex(dl_sources[src_num][dl_type], charsmax(dl_sources[][dl_type]), ExplodedString[1]);
...
public Check_Download_Demos(Community, bool:Download, bool:Filetype)
{
	...
	{
		new Link[256];
		format(Link, charsmax(Link), "%s/lj/%s/%d_%s_%s.rar", g_szDemoFiles[Community][LINK], dl_sources[src_num][dl_type], dl_sources[src_num][dl_distance], dl_sources[src_num][dl_type], dl_sources[src_num][dl_name]);
		...
		new archivefile[128];
		format (archivefile, charsmax(archivefile), "%s/%d_%s_%s.rar", archive_dir, dl_sources[src_num][dl_distance], dl_sources[src_num][dl_type], dl_sources[src_num][dl_name]);
...
{	
	...
	{		
		...
		format (filename, charsmax(filename), "%s/%d_%s_%s.rar", archive_dir, dl_sources[src_num][dl_distance], dl_sources[src_num][dl_type], dl_sources[src_num][dl_name] );
		...
		if (iResponceCode > 399)
		{
			...
			#endif
			server_print("Can't Download archive %s from file %s", filename , kz_lj);
		}
		else
		{			
			#if defined DEBUG
	...
	while ( !feof( iDemosList ) )
	{
		...
		{			
			...
			if(equali(Mapa, g_szCurrentMap))
			{
				...
				#if defined DEBUG
					//server_print("Archivename %s_%s_%s", dl_sources[src_num][dl_mapname], dl_sources[src_num][dl_name], dl_sources[src_num][dl_time]);
				#endif
			}
		}
	}
...
@OnComplete(id, iError)
{
	...
	else
	{
		...
		if(kz_lj2)
		{
			format(destname, charsmax(destname), "%s/%d_%s_%s.dem", temp_dir, dl_sources[src_num][dl_distance], dl_sources[src_num][dl_type], dl_sources[src_num][dl_name]);
		}
		else
		{
		...
		if(is_valid_demo_file(file))
		{
			if(read_demo_header( file ))
			{
				...
				if(kz_lj2)
				{
					format(bot_sources[sources_num][source_name], 32, "%s", dl_sources[src_num][dl_name]);
					format(bot_sources[sources_num][source_time], 32, "%d_%s", dl_sources[src_num][dl_distance], dl_sources[src_num][dl_type] );
				}
				else
				{
...
public kzbot_settings(id)
{
	...
	if(kz_lj2)
	{
		format(sz_name, charsmax(sz_name), "[WR] %s %s" , bot_sources[plr_source[id]][source_name], bot_sources[plr_source[id]][source_time]);
	}
	else
	{
...
public save_run(id, demo_time[32], bool:is_demofile)
{
	...
	if(!is_demofile)
	{
		...
		client_print( id, print_chat, "A flag" )
