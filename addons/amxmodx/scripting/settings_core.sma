...
public native_set_option_cell() {
    ...
    ExecuteForward(g_Forwards[fwdOnCellValueChanged], dummy, id, optionId, newValue);

    if (notify) {
        ExecuteForward(g_Forwards[fwdOnNotifyMysqlCellValue], dummy, id, optionId, newValue);
    }
...
public native_set_option_string() {
    ...
    // notify other plugins that value has changed
    ExecuteForward(g_Forwards[fwdOnStringValueChanged], dummy, id, optionId, newValue);

    if (notify) {
        ExecuteForward(g_Forwards[fwdOnNotifyMysqlStringValue], dummy, id, optionId, newValue);
    }
...
public OnConnectionIsReady() {
    ...
    for (new i = 0; i < ArraySize(ga_CellOptions); ++i) {
        ...
        ExecuteForward(g_Forwards[fwdOnRegisterOption], dummy, optionName, fieldType, defaultValue);
    }
    ...
    ExecuteForward(g_Forwards[fwdOnRegisterOption], dummy, RESERVER_OPTION_END, 0, "");
}
