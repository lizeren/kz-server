...
public Pause(id)
{
	...
	else
	{
		...
		//set_pev(id,pev_flags,PauseFlag)
...
public GoCheck(id) 
{
	...
	// Retrive user's view angle
	set_pev(id, pev_v_angle, Player_view_angle[id] );
	set_pev(id, pev_angles, Player_view_angle[id]);
	set_pev(id, pev_view_ofs, Player_off_angle[id]);
	set_pev(id , pev_fixangle, 1 )
	...
	set_pev(id, pev_origin, New_Checkpoints[id][New_stuck_index[id]]);
	gochecknumbers[id]++
...
public Stuck(id)
{
	...
	New_stuck_index[id] -= 1
	if (New_stuck_index[id] == -1){
		New_stuck_index[id] = 4
	}
	set_pev( id, pev_velocity, Float:{0.0, 0.0, 0.0} )
	set_pev( id, pev_view_ofs, Float:{  0.0,   0.0,  12.0 })
	set_pev( id, pev_flags, pev(id, pev_flags) | FL_DUCKING )
...
public noclip(id)
{
	...
	if(IsPaused[id] && (get_pcvar_num(kz_noclip_pause) == 1))
	{
		...
		//by commenting these, players can move when the run is paused
...
public FwdHamPlayerSpawn( id )
{
	...
	if(firstspawn[id])
	{
		ColorChat(id, GREEN,  "%s^x01 Welcome to ^x03George's Server ^x01", prefix)
...
public SavePos(id)
{
	...
	// set_hudmessage(255, 128, 0, 0.40, 0.10, 0, 0.0, 1.0, 0.0, 0.0, 1)	
	// show_hudmessage(id, "Position Saved ") 
...
public kz_menu(id)
{
	...
	formatex(title, 63, "\yKreedz Menu\w")
